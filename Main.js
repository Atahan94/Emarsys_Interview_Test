 function CalculateDueDate(startdate, time)
 {
    if (!isValidDate(startdate)) {
      ...
      const workingDays = Math.floor(time / 8); // Number of full working days base on given turnaround time 
      ...
      dueDate = addRemainingHours(dueDate, remainingHours); // Adding remaining hours
...
function isValidDate(date) // To validate our recived data before our calculation to prevent errors 
{
  const dateFormat = /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/;
  return dateFormat.test(date);
}

function formatDate(date) { // Formats the date as String Date/Time
